    ...
    <main id="ana-icerik" className="dashboard-page" tabIndex={-1}>
      <header className="dashboard-header">
        <div>
          ...
          <h1>Banka transferi kuyruğu</h1>
        </div>
        <AdminNavigation platformRole={user.platformRole} />
      </header>
      {payments.length === 0 ? (
        <p className="form-status">Bekleyen banka transferi yok.</p>
      ) : (
        <section className="order-list" aria-label="Banka transferleri">
          {payments.map((payment) => (
            <article className="dashboard-card order-card" key={payment.id}>
              <div>
                <span className="status-pill">{payment.status}</span>
                <h2>{payment.order.publicNumber}</h2>
                <p>{payment.buyerOrganization.tradeName}</p>
              </div>
              <div>
                <strong>{formatTryMinor(payment.amountMinor)}</strong>
                <p>Referans: {payment.bankTransferReference}</p>
                <Link href={`/admin/odemeler/${payment.id}`}>Ödeme detayını aç</Link>
              </div>
            </article>
          ))}
        </section>
