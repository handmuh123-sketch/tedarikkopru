...
export default async function AdminPaymentDetailPage({ params }: Props) {
  ...
  const payment = await database.payment.findFirst({
    ...
    include: {
      order: { select: { publicNumber: true, status: true } },
  ...
  return (
    <main id="ana-icerik" className="dashboard-page order-detail" tabIndex={-1}>
      <header className="dashboard-header">
        <div>
          <p className="eyebrow">Banka transferi</p>
          <h1>{payment.order.publicNumber}</h1>
          <p>{payment.buyerOrganization.tradeName}</p>
        </div>
      ...
      <section className="dashboard-grid">
        <article className="dashboard-card">
          ...
          <h2>Ödeme özeti</h2>
          <p>Tutar: {formatTryMinor(payment.amountMinor)}</p>
          <p>Referans: {payment.bankTransferReference}</p>
          <p>Not: {payment.bankTransferNote ?? "Not eklenmedi."}</p>
        </article>
        <article className="dashboard-card">
          <BankTransferDecisionForm
            paymentId={payment.id}
            status={payment.status}
            canDecide={operatorRoles.includes(user.platformRole)}
          />
        </article>
      </section>
      <section className="dashboard-card">
        ...
        <ol className="status-history">
          <li>
            ...
            <span>
              Transfer bildirimi oluşturuldu ·{" "}
              {payment.createdAt.toLocaleString("tr-TR", { timeZone: "Europe/Istanbul" })}
            </span>
          </li>
          {payment.attempts.map((attempt) => (
            <li key={attempt.id}>
              ...
              <span>
                {attempt.createdAt.toLocaleString("tr-TR", { timeZone: "Europe/Istanbul" })}
              </span>
            </li>
          ))}
        </ol>
